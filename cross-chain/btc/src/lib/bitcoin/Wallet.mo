...
import P2pkh "lib/P2pkh";
import Bitcoin "lib/Bitcoin";
import Address "lib/Address";
import Transaction "lib/Transaction";
import Script "lib/Script";
...
module {
  type Network = ICBTC.Network;
  type BitcoinAddress = ICBTC.BitcoinAddress;
  type Satoshi = ICBTC.Satoshi;
  type Utxo = ICBTC.Utxo;
  ...
  public func get_p2pkh_address(network : Network, key_name : Text, derivation_path : [[Nat8]]) : async BitcoinAddress {
    ...
    let public_key = await ICBTC.ecdsa_public_key(key_name, Array.map(derivation_path, Blob.fromArray));
  ...
  public func send(network : Network, derivation_path : [[Nat8]], key_name : Text, dst_address : BitcoinAddress, amount : Satoshi) : async [Nat8] {
    ...
    let fee_percentiles = await ICBTC.get_current_fee_percentiles(network);
    ...
    let own_public_key = Blob.toArray(await ICBTC.ecdsa_public_key(key_name, Array.map(derivation_path, Blob.fromArray)));
    let own_address = public_key_to_p2pkh_address(network, own_public_key);

    Debug.print("Fetching UTXOs...");
    let own_utxos = (await ICBTC.get_utxos(network, own_address)).utxos;
    ...
    let signed_transaction_bytes = await sign_transaction(own_public_key, own_address, transaction, key_name, Array.map(derivation_path, Blob.fromArray), ICBTC.sign_with_ecdsa);
    ...
    await ICBTC.send_transaction(network, signed_transaction_bytes);
    Debug.print("Done");
