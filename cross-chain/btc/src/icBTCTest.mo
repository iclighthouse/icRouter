...
//record { totalSupply=0; decimals=8; fee=20; name=opt "icBTC (PR)"; symbol=opt "icBTC"; metadata=null; founder=null;}  
shared(installMsg) actor class DRC20(initArgs: Types.InitArgs) = this {
    ...
    */
    ...
    private stable var maxSupply_ : ?Nat = null; //ck
    ...
    * ck
    */
    public shared(msg) func ictokens_addMinter(_minter: Principal) : async Bool{ //ck
        assert(_onlyOwner(msg.caller));
        minters := AID.arrayAppend(minters, [_minter]);
        return true;
    };
    public shared(msg) func ictokens_removeMinter(_minter: Principal) : async Bool{  //ck
        assert(_onlyOwner(msg.caller));
        minters := Array.filter(minters, func (t: Principal): Bool{ t != _minter });
        return true;
    };
    public query func ictokens_minters() : async [Principal]{  //ck
        return minters;
    };
    ...
    public shared(msg) func ictokens_burn(_value: Amount, _nonce: ?Nonce, _sa: ?Sa, _data: ?Data) : async (result: TxnResult) { //ck
